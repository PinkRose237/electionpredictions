...
uv run electionpredictions ingest --only polls,markets  # some sources (races,ratings,generic,polls,fec,wiki_fundraising,markets,news)
...
| `OPENCODE_API_KEY` | https://opencode.ai/auth (OpenCode Zen; the default model tier is free) | AI race briefs on race pages, generated by Meta's Muse Spark 1.3 Contributor. Set `AI_MODEL=muse-spark-1.3` for the paid non-contributor tier. |
...
| `OPENCODE_API_KEY` | unset | Enables the optional AI race briefs (`ingest --only analysis`); `AI_MODEL` / `AI_BASE_URL` override the model and endpoint |
